    ...
    SBMA.processCCPayment = function (executionContext) {
        ...
        if (formType === Constants.FORM_TYPE_UPDATE) {
            if (fieldValue === 646150001) {
                var ccPayment = {
                    cardHoldersName: formContext.getAttribute("sbma_nameoncard").getValue(),
                    expiryDate: formContext.getAttribute("sbma_cardexpirydate").getValue(),
                    cardNumber: formContext.getAttribute("sbma_ccnumber").getValue(),
